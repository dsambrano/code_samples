...
x <- read.delim(file="http://psychology467.zxq.net/screen.dat", header=TRUE, sep="\t",row.names=1)
...
####4a. Normality check all plots on one screen
vars <- names(x)
for(i in 1:ncol(x)){
  par(mfrow=c(2,2))
  hist(x[,i], main=vars[i], xlab=vars[i])
  qqnorm(x[,i], col="red", main=vars[i])
  qqline(x[,i])
  plot(density(na.omit(x[,i])),ylab="Proportion",main=vars[i])
  polygon(density(na.omit(x[,i])),col="cyan")
  boxplot(x[,i], main=vars[i])
}
...
write.table(complete(MeanX),file="C:/Users/Kathleen/Dropbox/Psychology 467/Tabachnick and Fidell datasets/screen Mean Inputation.dat", row.names=FALSE, sep="/t")
RegX <- mice(normX,m=1,meth="norm") #Regression Imputation
write.table(complete(RegX),file="C:/Users/Kathleen/Dropbox/Psychology 467/Tabachnick and Fidell datasets/screen Regression Inputation.dat", row.names=FALSE, sep="/t")
...
summary(MIx)
write.amelia(MIx, separate=TRUE,file.stem="C:/Users/Kathleen/Dropbox/Psychology 467/Tabachnick and Fidell datasets/Amelia",format="csv")
